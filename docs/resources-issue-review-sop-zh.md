...
- 当前 issue 在图中的更新位置描述（至少包含方位、线路/支线名称、关键站段、换乘站或终点站）
...
- 正确无误时，结论写“通过”
- 有错误时，尽可能列出所有已确认且可复核的错误点；不要包含尚未确认、无法稳定复核或仅凭猜测得出的错误
- 错误点较多时，使用分条列表，每条都应对应官方图与更新图之间的具体差异
- 列出图中什么方位有怎样的更新（不包括新增的资源）
...
图中更新位置：<方位 + 线路/支线 + 关键站段；例如“底部中偏左，西善桥向南新增 S2/宁马线至太白”>
